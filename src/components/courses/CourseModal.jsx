...
const CourseModal = ({ open, mode, data, onClose, onSubmit }) => {
  ...
  const InputField = ({ label, name, type = "text", options, span = 1 }) => (
    <div className={`flex flex-col gap-1 ${span === 2 ? "col-span-2" : ""}`}>
      ...
      {options ? (
        ...
        <textarea
          ...
          className={`px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-pup-maroon/20 focus:border-pup-maroon transition resize-none ${
            errors[name] ? "border-red-400" : "border-gray-200"
          }`}
        />
      ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-5 grid grid-cols-2 gap-4 overflow-y-auto">
          <InputField label="Course Code"       name="code"             span={1} />
          <InputField label="Status"            name="status"           options={STATUSES} />
          <InputField label="Course Name"       name="name"             span={2} />
          <InputField label="Department"        name="department"       options={DEPARTMENTS} span={2} />
          <InputField label="Duration"          name="duration"         options={DURATIONS} />
          <InputField label="Year Established"  name="yearEstablished"  type="number" />
          <InputField label="Description"       name="description"      type="textarea" span={2} />
        </div>
